...
parser = argparse.ArgumentParser(
    description="Extract Teochew pronunciations from En Wiktionary"
)
parser.add_argument(
    "--dump", type=str, action="store", help="Article data dump from English Wiktionary"
)
parser.add_argument(
    "--scraped",
    type=str,
    action="store",
    help="JSON file already produced by scraping the data dump. If supplied, then argument to --dump is ignored",
)
parser.add_argument(
    "--out",
    type=str,
    default="teochew_scrape.json",
    help="Path to write scraped data, keyed by character",
)
parser.add_argument(
    "--out_pron_prefix",
    type=str,
    default="teochew_scrape_pron_",
    help="Prefix to write markdown files, sorted by head letter and pronunciation",
)
parser.add_argument(
    "--out_pron_suffix", type=str, default="md", help="Suffix for markdown files."
)
parser.add_argument(
    "--limit", type=int, default=-1, help="Stop after the first N entries in the input"
)
...
for key in outdict.keys():
    for tprons in outdict[key]["mn-t"]:
        for tpron in re.split(
            "\s*/\s*", tprons
        ):  # Split on slash if alternative pronunciations given
            headletter = ""
