...
def convert_fipa2pengim(text):
    fipa2pengim = OrderedDict(
        [
            ("b", "bh"),  # Order of substitution is important
            ("p", "b"),
            ("ph", "p"),
            ("z", "r"),
            (r"t([^hs])", r"d\1"),
...
def convert_pengim2fipa(text):
    pengim2fipa = OrderedDict(
        [
            ("p", "ph"),  # Order is important
...
def convert_pengim2ipa(text):
    ...
    trn = {"a": "ã", "e": "ẽ", "i": "ĩ", "o": "õ", "u": "ũ", "ɯ": "ɯ̃"}
...
def convert_pengim_tonemarks(text):
    ...
    textsplit = re.split(
        r"([’\w]+?\d)", text
    )  # use ? for minimal match, e.g. for hak4hau6
    ...
    tr2 = str.maketrans(
        "AEIOUMNaeioumn", "ÀÈÌÒÙMǸàèìòùmǹ"
    )  # Syllable either has vowel, or is m or ng
            ...
            if int(im.group(2)) == 2:
                if im.group(1).lower() == "m":  # Edge case: m2 (no `m symbol available)
                    ...
                    out.extend(
                        re.sub(
                            r"[Nn]",
                            lambda x: x.group().translate(tr2),
                            im.group(1),
                            count=1,
                        )
                    )
                ...
                if (
                    im.group(1).lower() == "m"
                ):  # Edge case: m3 or m7, no ˇm symbol available
                    out.extend(i)
                elif im.group(1).lower() == "ng":  # avoid situation like ňga
                    out.extend(
                        re.sub(
                            r"[Nn]",
                            lambda x: x.group().translate(tr37),
                            im.group(1),
                            count=1,
                        )
                    )
                ...
                if (
                    im.group(1).lower() == "m" or im.group(1).lower() == "ng"
                ):  # Edge cases m5 and ng5
                    out.extend(i)
                else:
                    out.extend(
                        re.sub(
                            r"[AEIOUaeiou]",
                            lambda x: x.group().translate(tr58),
                            im.group(1),
                            count=1,
                        )
                    )
                ...
                if im.group(1).lower() == "m":
                    ...
                    out.extend(
                        re.sub(
                            r"[Nn]",
                            lambda x: x.group().translate(tr6),
                            im.group(1),
                            count=1,
                        )
                    )
...
def parse_wiktionary_dump(args):
    ...
    for event, elem in ET.iterparse(fh, events=("start", "end")):
        ...
        if (
            event == "end" and elem.tag == namespace + "page"
        ):  # Use endswith because tags have a file-specific prefix
            counter += 1  # Count each page = each entry
            if counter % 50000 == 0:
                print("Processed " + str(counter) + "pages")
            if (
                args.limit > 0 and counter >= args.limit
            ):  # Break out of loop - for previewing
                elem.clear()
                break
            ...
            if (
                not currtitle.startswith("MediaWiki")
                and not currtitle.startswith("Wiktionary")
                and not currtitle.startswith("Template")
                and not currtitle.startswith("Module")
            ):
